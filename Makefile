...
# 2. Busy-Box setup
busybox-compile: $(FIRST_RUN_FLAG)
...
init-script-vehicle: $(VEHICLE_BUILD_DIR)/$(VEHICLE_TARGET) busybox-compile 
	...
	@echo "QUADRANT=$$(cat /proc/cmdline | sed -n "s/.*quadrant=\([^ ]*\).*/\1/p")" >> $(VEHICLE_BUILD_DIR)/init
	...
	@echo "./$(VEHICLE_TARGET) $(COMPS) $$QUADRANT /mnt/host_logs/$$LOG_FILE" >> $(VEHICLE_BUILD_DIR)/init
